...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 std::vector< double > getyPlotData();
 ...
 std::vector< double > getxPlotData();
...
private:
 ...
 bool
  is_convex_iteration; //flag to tell whether at least one convex cut was found
 ...
 std::vector< double > xPlot; ///< abscissae computed by getxPlotData()
 ...
 std::vector< double > SPLabels;  //dual values of SP
 ...
 int LineSearch();  // returns the position in Q of the optimal intersection
                    // between the cuts
 ...
 void UpdCut( double alpha , double beta ,
              int i ); // support method for the LS.


 /// deep-copies flow/links/nodes data into the Flow/Links/Nodes members
 void copyDataArray( DCR::DCRFlow flow , std::vector< DCR::DCRLink > links ,
                     std::vector< DCR::DCRNode > nodes );
