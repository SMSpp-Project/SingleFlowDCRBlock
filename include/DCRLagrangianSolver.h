...
/*--------------------------- GENERAL NOTES --------------------------------*/
/*--------------------------------------------------------------------------*/
/// Lagrangian-relaxation solver for the Single-Flow Single-Path SRP DCR
/// problem
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 struct Cut_Val
 {
  ...
  std::vector< double > RSol; ///< the rate solution that generated the cut
...
/*--------------------------------------------------------------------------*/
 ...
 /// constructor of DCRLagrangianSolver: initializes all fields to default
 /// values
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 /// returns the positions, in the original graph, of the arcs *not* in the
 /// reduced graph
...
/*--------------------------------------------------------------------------*/
 ...
 /// checks whether the Lagrangian dual has a feasible solution, without
 /// solving it
...
/*--------------------------------------------------------------------------*/
 ...
 // FIXME: should this also require the desired precision? (implemented with
 // machine precision)

 void LoadProblem( int nnodes , int nlinks , DCR::DCRFlow flow ,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 ...
 void closeArcs( std::vector< int > arcs , int na );
...
/*--------------------------------------------------------------------------*/
 ...
 /// destructor of DCRLagrangianSolver: releases all dynamically allocated
 /// memory
...
/*--------------------------------------------------------------------------*/
 ...
 void copyDataArray( DCR::DCRFlow flow , DCR::DCRLink * links ,
                     DCR::DCRNode * nodes );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 /// computes, for the current lambda, the locally optimal rate and cost of
 /// every reduced-graph arc
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
 /// attempts to warm-start the line search from the cached cuts after r_min
 /// changed
...
protected:
 ...
 double
  lambda; ///< current/optimal Lagrangian multiplier (intersection point)
 ...
 std::vector< double >
  ModCaps; ///< working (possibly closed/reopened) arc capacities

 // data produced by the Shortest Path Tree solves
 int nhops; ///< number of arcs (hops) of the optimal path
 std::vector< DCR::DCRLink > RedGraLinks;  ///< the DCR arcs of the reduced
                                           ///< graph
 std::vector< int >
  RedGraPos; ///< positions of the reduced graph arcs in the full graph
 std::vector< int >
  ComplGraph;  ///< positions, in the full graph, of the arcs NOT in the
               ///< reduced graph
 std::vector< int >
  OptPath;       ///< positions (in the reduced graph) of the optimal path
 int solvedflag; ///< true if the current data have already been solved
 int
  inizialflag; ///< true if Inizial() has already been run on the current data
 int
  reoptflag; ///< true if Solve() should attempt Reopt() (r_min just changed)
 //double * XSol;
 int numHops; ///< number of arcs (hops) of the optimal path (see nhops)
 std::vector< double > RSol; ///< the r_ij of the optimal solution
 std::vector< double >
  RSolCosts; ///< the arc costs f_ij associated with the r*_ij of RSol
 std::vector< double >
  SPLabels; ///< Shortest Path Tree node potentials of the optimal solution

 int
  maxCutSize; ///< maximum number of cuts kept in the Cuts cache for Reopt()

 // auxiliary data used to build a CPLEX-comparable heuristic check solution
 std::vector< double > solpos;  ///< rate solution of the best positive-slope
                                ///< cut
 std::vector< double > solneg;  ///< rate solution of the best negative-slope
                                ///< cut
 ...
 std::vector< int > checkSolPos; ///< positions (in the graph) of solpos
 std::vector< int > checkSolNeg; ///< positions (in the graph) of solneg

 double
  eps; ///< relative precision used as convergence tolerance (machine epsilon)
 SPT spt; ///< the Shortest Path Tree solver used by the line search
 std::vector< SPT::SPTLink > Linksp;  ///< simplified arcs (cost only) fed to
                                      ///< spt
 SPT::Status spstat;            ///< outcome of the last spt.Solve() call
 LAGStatus
  lagstat; ///< outcome of the Lagrangian relaxation (see getStatus())
