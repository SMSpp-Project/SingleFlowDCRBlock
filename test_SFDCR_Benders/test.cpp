...
SingleFlowDCRBlock * oMCFB1 = nullptr;    // original SingleFlowDCRBlock
...
static void load1( string fndmx, string fndcr ){
 ...
 ifstream fndcr1(fndcr);
 oMCFB1->load( fndmx1 );
 Index NNodes = oMCFB1->get_NNodes();
 Index NArcs = oMCFB1->get_NArcs();
 try {  
   oMCFB1->load_dcr( fndcr1 , NNodes , NArcs );
  } 
  catch(...) {
    cerr << "Error: dcr file error!" << endl;
    exit( 1 );
...
int main( int argc , char **argv )
{
 ...
 for(j=0; j<NComm; j++){
  ...
  auto bsc = dynamic_cast< BlockSolverConfig * >(Configuration::deserialize( "Bender.txt" ) );
  auto bsc1 = dynamic_cast< BlockSolverConfig * >(Configuration::deserialize( "MILPPar2.txt" ) );
  if( ! bsc ) {
  ...
  oMCFB1 = dynamic_cast< SingleFlowDCRBlock * >( Block::new_Block( "SingleFlowDCRBlock" ) );
  assert( oMCFB );

  load( "output.dmx", "output.dcr" );
  load1( "output.dmx", "output.dcr" );
  ...
  Solver * slvr1 = oMCFB1->get_registered_solvers().front();
  ...
  */
  ...
  std::cout << "computed commodity " << j << std::endl;

  if(abs(slvr->get_lb() - slvr1->get_lb()) > 10.0)
    std::cout << "ERROR! " << j << ":" << slvr->get_lb() << "-" << slvr1->get_lb() << " " 
      << (slvr1->get_lb()-slvr->get_lb())/slvr1->get_lb() << "\n";
