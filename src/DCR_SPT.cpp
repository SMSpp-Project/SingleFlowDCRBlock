...
int DCR_SPT::DCRgetUBSolNPaths( int k )
{
 if( k < 0 || k >= numFlows )
  throw(
   DCR::DCRException( "DCR_SPT::DCRgetUBSolNPaths(): wrong flow index." ) );
 if( UBsol )
  return 1;
 else
  throw( DCR::DCRException( "DCR_SPT::DCRgetUBSolNPaths(): no solution "
                            "found." ) ); //FIXME: is this okay???
 }
...
int DCR_SPT::DCRgetUBSolPath( int k , int p , int * X , double * R )
{
 ...
 if( k < 0 || k >= numFlows )
  throw(
   DCR::DCRException( "DCR_SPT::DCRgetUBSolPath(): wrong flow index." ) );
 if( p != 0 )
  throw(
   DCR::DCRException( "DCR_SPT::DCRgetPUBolPath(): wrong path index." ) );
 if( UBsol ) {
  ...
  throw( DCR::DCRException( "DCR_SPT::DCRgetUBSolPath(): no solution "
                            "found." ) ); //FIXME: is this okay ???
...
{
 throw( DCR::DCRException( "DCR_SPT::DCRgetPSolNPaths(): Continuous "
                           "relaxation is not supported...sorry." ) );
 }
...
{
 throw( DCR::DCRException( "DCR_SPT::DCRgetPSolPath(): Continuous relaxation "
...
void DCR_SPT::DCRloadProblem( int nnodes , int nlinks , int nflows ,
                              ...
                              DCRNode * nodes , double mtu , DCRDelay deltype )
{
 int status;

 if( deltype != SRP )
  throw( DCR::DCRException( "DCR_SPT::DCRloadProblem(): this delay formula "
                            "is not supported yet..sorry" ) );

 if( nflows > 1 )
  throw( DCR::DCRException( "DCR_SPT::DCRloadProblem(): this class can only "
                            "solve SFSP DCR problems..sorry" ) );
...
void DCR_SPT::DCRsetHeur( char h )
{
 if( h == '1' )
  ...
  throw( DCR::DCRException(
   "DCR_SPT::DCRsetHeur(): this heur. type is not supported yet..sorry" ) );
 }
...
/* This private method is used to copy array data (flows, links and nodes)
 * from the load() function
 * into the class as class members.*/
void DCR_SPT::copyDataArrays( DCRFlow * flows , DCRLink * links ,
                              DCRNode * nodes )
{
 int i , j , k;
